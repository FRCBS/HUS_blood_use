...
-- NB! --
Updated 21.10.2024 due to change of journal. Changes:
1. DRGs removed from Table 1 into their own bar chart.
2. Colored all figures.
...
No longer part of Table 1. Its own Figure now. Number unknown at the moment.

```{r}
fill_colors <- c("#e7298a", "#00BFB2", "#e6ab02")

pdf_drg <- master %>%
    filter(!is.na(code_drg) & !is.na(type_product) & sex_patient != "Ei tiedossa/Määriteltävissä") %>%
    group_by(code_drg, sex_patient, type_product) %>%
    summarise(drg_count = n()) %>%
    group_by(code_drg) %>%
    mutate(sum = sum(drg_count)) %>%
    ungroup() %>%
    arrange(desc(sum)) %>%
...
pdf_drg_topn <- pdf_drg %>%
    filter(code_drg %in% drg_order$code_drg[1:15])

# Add labels manually
pdf_drg_topn <- pdf_drg_topn %>%
    mutate(label_drg = case_when(
        code_drg == "817O" ~ "Non-ext. procedure of myeloprolif. disease",
        code_drg == "395" ~ "Red blood cell disorders",
        code_drg == "473" ~ "Acute leukemia w/o major OR procedure",
        code_drg == "816O" ~ "Non-ext. procedure of blood and imm. disease",
        code_drg == "174N" ~ "Complex peptic ulcer / GI-hemorrhage",
        code_drg == "916O" ~ "Disease or disorder of blood forming organs or imm. disorder",
        code_drg == "480" ~ "Liver transplanation",
        code_drg == "210N" ~ "Pelvic, hip and femur procedures ex. major joint with CC",
        code_drg == "110" ~ "Major cardiovascular procedures with CC",
        code_drg == "148" ~ "Major small and large bowel procedures with CC",
        code_drg == "104B" ~ "Multiple heart valve / single valve operations with CC",
        code_drg == "478" ~ "Other vascular procedures with CC",
        code_drg == "483B" ~ "Intensive circulatory support w/o heart or lung transplantation",
        code_drg == "175N" ~ "Peptic ulcer / GI-hemorrhage w/o CC",
        code_drg == "917O" ~ "Myeloprolif. disease or disorder or poorly diff. neoplasms",
    ))


discrete_y <- unique(pdf_drg_topn$label_drg)
# Break long labels
discrete_y_split <- splitAtCentralWhitespace(discrete_y, 33)

pdf_drg_topn_py <- pdf_drg_topn %>%
  mutate(x_transformed = ifelse(sex_patient == "Female", 100 * drg_count / nrow(master) * -1, 100 * drg_count / nrow(master)))
...
dummy_df <- data.frame(
  label_drg = unique(pdf_drg_topn$label_drg),
  x_transformed = -2.25, # edit this?
  type_product = NA,
  sex_patient = "Female"
)

ggplot(pdf_drg_topn_py,
    aes(
        y = factor(label_drg, levels = rev(discrete_y)), 
        x = x_transformed, 
        fill = type_product
    )
    ) +
    ...
    scale_fill_manual(values = fill_colors, breaks = c("RBC", "Platelets", "Plasma")) +
    facet_wrap(vars(sex_patient), scales = "free_x") +
    theme_minimal() +
    labs(x = "Percent of all transfused units", y = "Diagnosis Related Group", fill = "Product type") +
    theme(legend.position = "bottom", text = element_text(size = 13), panel.spacing = unit(0, "lines")) +
    scale_x_continuous(labels = abs)


ggsave("./results/manuscript1_plots/Figure DRG.png", width = 3000, height = 3000, units = "px", bg = "white")
ggsave("./results/manuscript1_plots/svg/Figure DRG.svg", width = 3000, height = 3000, units = "px", bg = "white")
```
...
ggplot(hbs, aes(x = Hb, fill = factor(variable, levels = c("pre", "post")))) +
    ...
    scale_fill_manual(values = rev(fill_colors[1:2]), name = "", labels = c("Before transfusion", "After transfusion")) +
...
bleeds <- master %>%
    ...
    #group_by(id_patient) %>%
    #arrange(desc(episode_start)) %>%
    #mutate(episdiff = difftime(episode_start, lag(episode_end), units = "hours")) %>%
    #ungroup() %>%
    ...
    #mutate(blood_volume = weight_patient * 70 / sqrt(bmi / 22)) %>% # LBB equation
    mutate(sex_group_patient = case_when(sex_group_patient == "mies" ~ "Male",
                                  sex_group_patient == "nainen_postmeno" ~ "Female | Postmenopausal",
                                  sex_group_patient == "nainen_premeno" ~ "Female | Premenopausal")) %>%
    group_by(id_transfep) %>%
...
ggplot(bleeds, aes(x = bloodloss_pct_per_unit)) +
    geom_density(fill = "#e7298a", alpha = 0.5) +
    facet_wrap(vars(sex_group_patient), ncol = 1) +
    xlim(c(0, 20)) +
...
```{r}
fill_colors <- c("#e7298a", "#00BFB2", "#4e4553") # With black
# fill_colors <- c("#e7298a", "#00BFB2", "#e6ab02") # with yellow

color_colors <- c("#c71670", "#008c82", "#342e37") # With black
# color_colors <- c("#c71670", "#008c82", "#b38502") # with yellow
